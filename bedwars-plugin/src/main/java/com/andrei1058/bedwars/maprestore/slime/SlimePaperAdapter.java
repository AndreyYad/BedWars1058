...
import com.andrei1058.bedwars.util.FileUtil;
import com.andrei1058.bedwars.util.ZipFileUtil;
...
public class SlimePaperAdapter extends RestoreAdapter {
    ...
    public SlimePaperAdapter(Plugin plugin) {
        super(plugin);
        slime = (SlimePlugin) PluginManagerHelper.getPlugin("SlimeWorldManager");
    }

    @Override
    public void onEnable(@NotNull IArena a) {
        if (BedWars.nms.getMainLevel().equalsIgnoreCase(a.getWorldName())) {
            if (!(BedWars.getServerType() == ServerType.BUNGEE && Arena.getGamesBeforeRestart() == 1)) {
                FileUtil.setMainLevel("ignore_main_level", BedWars.nms);
        ...
        Bukkit.getScheduler().runTaskAsynchronously(getOwner(), () -> {
            ...
            try {
                ...
                Bukkit.getScheduler().runTask(getOwner(), () -> {
                    ...
                    PluginManagerHelper.callEvent(new WorldInitEvent(w));
                    Bukkit.getScheduler().runTask(getOwner(), () -> PluginManagerHelper.callEvent(new WorldLoadEvent(w)));
                });
    ...
    public void onRestart(IArena a) {
        if (BedWars.getServerType() == ServerType.BUNGEE) {
            if (Arena.getGamesBeforeRestart() == 0) {
                if (Arena.getArenas().size() == 1 && Arena.getArenas().get(0).getStatus() == GameState.restarting) {
                    getOwner().getLogger().info("Dispatching command: " + BedWars.getMainConfig().getString(ConfigPath.GENERAL_CONFIGURATION_BUNGEE_OPTION_RESTART_CMD));
                    Bukkit.dispatchCommand(Bukkit.getConsoleSender(), BedWars.getMainConfig().getString(ConfigPath.GENERAL_CONFIGURATION_BUNGEE_OPTION_RESTART_CMD));
                }
            } else {
                ...
                Bukkit.getScheduler().runTask(getOwner(), () -> {
                    ...
                    if (Arena.canAutoScale(a.getArenaName())) {
                        Bukkit.getScheduler().runTaskLater(getOwner(), () -> Arena.loadArena(a.getArenaName(), null), 80L);
                    }
                });
            }
        } else {
    ...
    private void convertWorld(String name, Player player) {
        ...
        try {
            ...
            if (player != null) {
                ...
                ISetupSession s = SetupSession.getSession(player.getUniqueId());
                if (s != null) {
                    s.close();
                }
            }
