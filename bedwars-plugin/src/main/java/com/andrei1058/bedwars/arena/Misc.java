...
public class Misc {
    ...
    public static boolean isProjectile(Material i) {
        return Material.EGG == i || Material.FIRE_CHARGE == i || Material.SNOWBALL == i || Material.ARROW == i;
    }
    ...
    public static void openStatsGUI(Player p) {
        ...
        Bukkit.getScheduler().runTask(plugin, () -> {
            ...
            for (String s : config.getYml().getConfigurationSection(ConfigPath.GENERAL_CONFIGURATION_STATS_PATH).getKeys(false)) {
                ...
                ItemStack i = nms.createItemStack(config.getYml().getString(ConfigPath.GENERAL_CONFIGURATION_STATS_ITEMS_MATERIAL.replace("%path%", s)).toUpperCase(), 1);
                ItemMeta im = i.getItemMeta();
